...
- [ ] Mejorar experiencia de edición de `tabla_datos`: admin puede agregar/quitar filas y columnas individualmente, definir tipo de celda (editable vs solo-lectura) por celda — solicitud de ususario para experiencia tipo Excel, celdas desbloqueables con `expression` `calculado` VS-043, persistencia de estructura. Siguiente slice tras VS-045.
