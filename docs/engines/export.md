...
- Una fila por **Elemento tipo pregunta** (`isQuestion: true` en `component-registry.ts`) de cada Subindicador del snapshot de la Evaluación, con columnas: `Dimensión`, `Indicador`, `Subindicador`, `Número` (VS-021, ver `../domain/evaluation-hierarchy.md`/`form.md` — `0.N`, reinicia en cada Subindicador, vía `questionNumber`), `Elemento`, `Tipo`, `Respuesta`, `Estado` (VS-018, ver `persistence.md` — `not_started`/`in_progress`/`completed`/`approved`/`submitted` vía `deriveStatus`), `Comentario confidencial` (VS-019, ver `persistence.md` — vacío si no hay comentario; "confidencial" es una etiqueta de UI, no una restricción de acceso, se exporta igual que cualquier otra respuesta).
