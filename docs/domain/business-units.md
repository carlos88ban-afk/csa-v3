...
> **Estado de implementación (2026-08-18)**: feature completa, VS-050 a VS-059, **todo verificado end-to-end en producción** (`csa-v3-web.vercel.app`). Backend: VS-050 schema/assignments, VS-051 partición de `response`, VS-052 `dueDate`/bloqueo, VS-053 acceso autenticado + exclusiones. UI: VS-054 Runtime compartido público/autenticado (`RuntimeCore`/`RuntimeAdapter`) + panel Publicar, VS-055 export XLSX consolidado + dashboard de avance por unidad, VS-057 bloqueo proactivo del formulario en cliente por `dueDate` vencido, VS-058 rutas de evidencia autenticadas (espejo de las públicas), VS-059 editor de exclusiones por Subindicador/elemento en el panel Publicar. Sin pendientes conocidos de alcance central.
...
- Nuevo botón "Publicar" en la barra superior del Builder → abre un panel (modal o drawer, a decidir en implementación) con:
  ...
  - Sección "Unidades de negocio" (solo si la organización actual tiene hijas): checklist para asignar/desasignar unidades a esta Evaluación — ✅ (vía `GET /api/organizations/children` + `GET/POST /api/evaluations/[id]/assignments` + `DELETE`); progreso por unidad (pills de porcentaje + "Plazo vencido") — ✅ completado en VS-055 (`getBusinessUnitProgress` + `GET /api/evaluations/[id]/progress`); por cada unidad asignada, un editor para marcar qué Subindicadores/elementos excluir (árbol Dimensión→Indicador→Subindicador, checkbox por nodo + por pregunta al expandir) — ✅ completado en VS-059 (`components/exclusion-editor.tsx` + `GET/POST .../exclusions` + `DELETE .../exclusions/[exclusionId]`)
...
## Testing (implementado VS-050 → VS-059)
...
- Verificación manual en producción: flujo completo con una organización matriz + unidad(es) de negocio, con exclusiones (de Subindicador completo y de pregunta individual) confirmando aislamiento visual y de datos entre unidades, y que la matriz sí ve todo — ✅ completo (VS-054: aislamiento de respuestas, bloqueo del link público en modo corporativo, 404 genérico para no-asignadas; VS-055: export XLSX y dashboard de progreso con aislamiento en vivo; VS-057: bloqueo de cliente por plazo vencido, en DOM real y con 403 de servidor confirmado; VS-058: evidencia autenticada — upload/descarga/borrado real más rechazo de elemento excluido; VS-059: exclusión de Subindicador y de pregunta individual reflejadas en dashboard y Runtime autenticada, incluyendo re-inclusión).
