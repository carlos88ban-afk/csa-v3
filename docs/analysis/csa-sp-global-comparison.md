...
**Actualización 2026-08-14 (6.ª inspección, HTML real de la pregunta `COG_BoardIndependence_AttachmentBoardIndependenceStatement` enviado por el usuario):** el texto de la pregunta y de las opciones lleva **formato** (`<strong>` y múltiples `<p>` dentro de los labels — ej. "Las empresas **cotizadas** deben…" / "Las empresas **no cotizadas** están obligadas a…"), algo que la plataforma no puede replicar: todos los labels se editan con `<input>` plano y se renderizan como texto corrido. Además, la fila de referencias de la opción "Sí" es `data-ref-type="flexible"` con `data-maxrefs="3"` — el ítem menor de la 5.ª inspección: admite URL pública O documento interno, mientras VS-039 modeló solo URLs públicas. Alcance confirmado con el usuario (`AskUserQuestion`): rich text en **todas** las preguntas y opciones, y **sí** incluir referencias flexibles en el mismo slice. Spec **VS-045** en `docs/engines/form.md` (antecede a VS-044/VS-043 por pedido explícito del usuario). El HTML también muestra un segundo grupo de checkboxes ("Distribución de objetivos") tras el sub-radio y un `<ol>` con sub-opciones que traen campos embebidos — ya construibles con VS-016/VS-040 (sin gap nuevo). Nota: VS-042 (tabla embebida en sub-opción) quedó **implementado y verificado** en la misma fecha — ver CHANGELOG; las specs VS-043/VS-044 siguen pendientes.

**Corrección 2026-08-15:** el párrafo anterior sobre "Distribución de objetivos" era incorrecto — una lectura apresurada del DOM que no siguió el prefijo de los `id`. Re-análisis pedido explícitamente por el usuario ("analiza esta pregunta... valida en producción") mostró que ese grupo de checkboxes NO está anidado dentro del sub-radio de StockExchange: es un `<ol>` **hermano**, independiente, con su propio encabezado, bajo la misma opción "Applicable". `formOption` solo admite un bloque `subOptions` (con una sola exclusividad para todo el array) — no hay forma de adjuntar un segundo bloque independiente a la misma opción. Validado tanto contra el schema (`form-schema.ts`) como en vivo contra el Builder de producción (framework temporal, borrado al terminar). Gap real, spec en `docs/engines/form.md` ("Bloque secundario de sub-opciones por opción, VS-046, pendiente").
